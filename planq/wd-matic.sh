...
if [ "$ANSWER" == "yes" ]; then
    ...
    echo "-------------------------------------------------------------------"
    echo $PASS | ${BINARY} tx distribution withdraw-rewards ${VALOPER} --commission --from ${KEY_NAME} --gas auto --gas-adjustment 1.5 --chain-id=${CHAIN} --gas-prices ${FEE} --node tcp://localhost:${RPC_PORT} -y | grep "raw_log\|txhash"
    ...
    if [[ $DELEGATE > 0 && $DELEGATE != "null" ]]; then
        ...
        echo "-------------------------------------------------------------------"
        echo $PASS | ${BINARY} tx staking delegate ${VALOPER} ${DELEGATE}${COIN} --chain-id=${CHAIN} --from ${KEY_NAME} --gas auto --gas-adjustment 1.5 --gas-prices ${FEE} --node tcp://localhost:${RPC_PORT} -y | grep "raw_log\|txhash"
        sleep 30s
        echo "-------------------------------------------------------------------"
