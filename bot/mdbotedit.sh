...
active_rolls=$(./massa-client wallet_info -p $PASSWORD | grep "Active rolls" | awk '{ print $3 }')
int_rolls=${active_rolls}

# update

final_balance=$(./massa-client wallet_info -p $PASSWORD | grep "Final balance" | awk '{ print $3 }')
int_final_balance=${final_balance}
candidate_balance=$(./massa-client wallet_info -p $PASSWORD | grep "Candidate balance" | awk '{ print $3 }')
int_candidate_balance=${candidate_balance}
locked_balance=$(./massa-client wallet_info -p $PASSWORD | grep "Locked balance" | awk '{ print $3 }')
int_locked_balance=${locked_balance}
final_rolls=$(./massa-client wallet_info -p $PASSWORD | grep "Final rolls" | awk '{ print $3 }')
int_final_rolls=${final_rolls}
candidate_rolls=$(./massa-client wallet_info -p $PASSWORD | grep "Candidate rolls" | awk '{ print $3 }')
