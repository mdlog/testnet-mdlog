...
if  [ $int_rolls -gt "0" ]; then
        curl -s -X POST "https://api.telegram.org/bot$TOKEN_BOT/sendmessage" -d "chat_id=$ID" -d "parse_mode=html" -d "text= ✅ NODE: $ADDRESS Aktif\Final balance: $int_final_balance\Candidate balance: $int_candidate_balance\Locked balance: $int_locked_balance\Active rolls: $int_rolls\Final rolls: $int_final_rolls\Candidate rolls: $int_candidate_rolls"
elif [ $int_rolls  -lt "1" ]; then
        curl -s -X POST "https://api.telegram.org/bot$TOKEN_BOT/sendmessage" -d "chat_id=$ID" -d "parse_mode=html" -d "text= ❌ NODE: $ADDRESS Tidak Aktif\Final balance: $int_final_balance\Candidate balance: $int_candidate_balance\Locked balance: $int_locked_balance\Active rolls: $int_rolls\Final rolls: $int_final_rolls\Candidate rolls: $int_candidate_rolls"
fi
   printf "sleep"
